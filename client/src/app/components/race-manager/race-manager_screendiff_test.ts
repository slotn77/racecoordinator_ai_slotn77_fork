...
test.describe("Race Manager Visuals", () => {
  ...
  test("should display race manager correctly", async ({ page }) => {
    ...
    const harness = new RaceManagerHarnessE2e(
      page.locator(RaceManagerHarnessBase.hostSelector),
    );
  ...
  test("should select and scroll to race from query param", async ({
    page,
  }) => {
    ...
    await TestSetupHelper.waitForLocalization(
      page,
      "en",
      page.goto("/race-manager?id=r25&driverCount=4"),
    );
    await TestSetupHelper.disableAnimations(page);

    // Verify Race 25 is active
    const activeRow = page.locator(
      `${RaceManagerHarnessBase.selectors.selectedItem}[data-id="r25"]`,
    );
